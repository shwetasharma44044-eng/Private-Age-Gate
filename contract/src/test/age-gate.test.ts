...
describe("AgeGate smart contract", () => {
  it("allows verification when age is above or equal to threshold", () => {
    ...
    const simulator = new AgeGateSimulator(20);
    const threshold = 18;
    const timestamp = BigInt(Date.now());

    const success = simulator.verifyEligibility(user, threshold, Number(timestamp));
  ...
  it("fails verification when age is below threshold", () => {
    const user = randomBytes(32);
    const simulator = new AgeGateSimulator(16);
    const threshold = 18;
    const timestamp = BigInt(Date.now());

    expect(() => {
      simulator.verifyEligibility(user, threshold, Number(timestamp));
  ...
  it("does not leak the private age value into the public ledger state", () => {
    const user = randomBytes(32);
    const simulator = new AgeGateSimulator(25);
    const threshold = 18;
    const timestamp = BigInt(Date.now());
